/*=====================================================================
*
* Template::Stash::Strict (Strict.xs)
*
* DESCRIPTION
...
#define TT_STASH_PKG	"Template::Stash::Strict"
...
static SV *dotop(pTHX_ SV *root, SV *key_sv, AV *args, int flags) {
    ...
    if (!result || !SvOK(result) || (result == &PL_sv_undef)) 
        die("Error fetching item: did not find key %s", item);
...
MODULE = Template::Stash::Strict		PACKAGE = Template::Stash::Strict
...
performance(verbose)
    ...
    perf_out.outsv = newSVpvf(
      TT_STASH_PKG " " Strict_VERSION " - Performance Summary for PID %d\n"
      "===================================================================\n"
